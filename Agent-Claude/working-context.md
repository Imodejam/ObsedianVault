...
### [2026-06-11] msg 3440 — Nuovo progetto Trading Bot + MCP trader-dev
- FATTO: `claude mcp add --transport sse --scope user trader-dev https://mcp.trader.dev/sse` → aggiunto a /home/claudebot/.claude.json, stato √ Connected. Scope user (tutti i progetti). NB: i tool MCP si caricano all'avvio sessione → disponibili dalla PROSSIMA sessione, non in questa.
- EFFETTO COLLATERALE: l'aggiunta ha ricaricato i server MCP e il tool Telegram (mcp__plugin_telegram_telegram__reply) si è scollegato DALLA MIA SESSIONE (server globale resta √ Connected). Non posso rispondere via Telegram finché il canale non si riconnette (al prossimo messaggio in arrivo). RICORDA: appena torna, inviare a Stefano le domande di scoping sotto.
