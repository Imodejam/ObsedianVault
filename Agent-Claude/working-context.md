...
- 🔧 CAUSA "DB GIÙ" TROVATA (2026-06-22): la vetrina (.env.local) E la PWA (piracity/.env + frontend/.env) puntano ancora al Supabase LEGACY supabase-cat.duckdns.org, DISMESSO il 17/05. Nuova infra CAT viva: API Piracity = https://api-cat.piracity.app (PostgREST+GoTrue, testato 200; Accept-Profile piracity). db-cat.puntify.it = DbGate UI (NON l'API, anche se Stefano l'ha indicato). Anon key CAMBIATA (JWT secret nuovo, la vecchia dà "invalid token"). Chiavi in /opt/ops/.env su server remoto pro-open (212.227.21.104) — non accessibile da questo box. RISOLTO 2026-06-22: chiavi nuove inserite in .env.local (anon role:anon, service-role role:service_role, exp 2036), servizio riavviato. DB RICONNESSO ✅: pagine città mostrano mappe (es. /it/citta/roma → "Cuore Eterno" 5 tappe). service-role testato ok. Nota: la NUOVA home NON elenca mappe (monta solo le 14 sezioni landing, niente CuratedMaps) → "0 /mappe/ sulla home" è BY DESIGN, non un bug.
⚠️ MARKETPLACE VUOTO: le 3 mappe published+official (Roma, Cosenza, Shanghai) hanno price=NULL → 0 vendibili. Serve impostare i prezzi (posso scriverli via service-role). Chiesto a Stefano gli importi.
Note minori: cover mappa rotte (cover_url assoluto su vecchio piracity-dev-app.duckdns.org); pagine città/dettaglio ancora dark (non ridisegnate).
---
Stefano CONFERMA: vetrina deve SEMPRE puntare all'API https://api-cat.piracity.app (NON db-cat). FATTO: aggiornato .env.local NEXT_PUBLIC_SUPABASE_URL→api-cat.piracity.app + next.config remotePatterns (api-cat + app-cat). Verificato: vecchie anon+service_role danno JWSInvalidSignature sul nuovo endpoint (JWT secret cambiato). ANCORA IN ATTESA da Stefano: PIRACITY_ANON_KEY + SERVICE_ROLE nuove → poi metto in .env.local + restart servizio → mappe tornano. Stessa fix poi sulla PWA (piracity/.env + frontend/.env puntano ancora al legacy).
