...
- Info-tooltip "i" FATTO: helper InfoTip + wwwroot/js/pan-tips.js (vanilla, event delegation hover/focus/tap, bolla fixed clampata nel viewport, flip sopra/sotto, no dip.) + index.html. Su ogni KPI + titolo card. Testi su calcolo reale, 16 chiavi pan_tip_* ×10 lingue. Build ok, pan-tips.js servito (200).
- IN CORSO (subagent a3ebef9424307e252, 2026-07-06 16:16): perf — fetch in parallelo (Task.WhenAll) per bundle, render progressivo (KPI negozio d'ingresso subito), lazy-load sezioni pesanti (Richieste AI/nemi history), cache periodo; + skeleton loaders shimmer per KPI/grafico/card al posto dello spinner. TODO server-side aggregation resta.
- PERF+SKELETON FATTO (subagent a3ebef9424307e252, 2026-07-06 16:20): fetch paralleli (Task.WhenAll) per bundle+tickets; render progressivo (PV d'ingresso subito, resto in background); Nemi history LAZY (LoadNemiForActiveAsync, box AI in skeleton _aiPending); bookings range ~62gg; cache _bundleCache (cambio periodo = ricalcolo in memoria, no refetch); skeleton .skeleton-shimmer/.pan-sk per KPI/grafico/card, no salto layout. Rimosso _aggLoading full-spinner. Build ok, app riavviata. TODO server-side: GetAllShopTransactions/nemi-history senza range, aggregazione multi-PV client-side.
