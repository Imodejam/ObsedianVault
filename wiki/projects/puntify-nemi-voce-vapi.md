...
## FASE 2 FATTA (subagent ac58cade9ca028f58, 2026-07-07) — tool voce prenotazioni
Formati Vapi verificati: tool = assistant.tools[] {type:function, function{name,desc,parameters}, server{url,secret}}; evento tool-calls (toolCallList o toolCalls OpenAI-form); risposta {results:[{toolCallId,result}]} HTTP 200 result parlabile.
NemiVoiceToolService: tool elenca_servizi/verifica_disponibilita/crea_prenotazione/trova_mie_prenotazioni/sposta_prenotazione/cancella_prenotazione (italiano, JSON-schema). Handler tool-calls in VapiWebhookController esegue via BookingServiceImpl (GetServicesAsync/GetSlotsAsync/CreateVoiceBookingAsync nuovo/RescheduleBookingAsync nuovo/CancelBookingAsync) + CustomerProfileService. Prenotazione voce: nome+telefono obbligatori (tel da chiamata), email opz, GdprConsent=true nota "informativa a voce", SourceChannel=voice, profilo segregato upsert. Riconoscimento in assistant-request: message.customer.number→FindByPhoneAsync→system prompt "CHIAMANTE RICONOSCIUTO" saluta per nome. Mini-informativa GDPR in BuildVoiceContextAsync (default hardcoded). Trova/sposta/cancella per telefono+indice 1-based. Build 0 errori, server :8001 riavviato.
LIMITI: date relative risolte dall'LLM; solo appuntamenti a operatore (no tavoli/asporto/risorse); testo informativa non ancora editabile UI; tool-calls non testati dal vivo (serve numero Vapi).
PRONTO PER TEST REALE: config Vapi (numero+ServerURL+Secret, numero in anagrafica shop a88d9724 con 60 min) → chiamare.

## 2026-07-07 (17:20) — chiarimento config Vapi: numero→Server URL (NON assistant fisso)
Stefano aveva assegnato l'assistant statico "Puntify - Nemi" al numero. GOTCHA: il nostro sistema è DINAMICO — Vapi deve chiederci l'assistant a ogni chiamata (assistant-request), che scatta SOLO se il numero NON ha un assistant fisso. Con assistant statico assegnato, assistant-request non scatta → si perde per-shop (orari/KB dinamici, gating saldo, tool da noi).
ISTRUZIONE data (msg 5497): sul NUMERO Vapi rimuovere l'assistant fisso + impostare Server URL=https://cat.puntify.it/api/vapi/webhook + Secret (o Server URL a livello Organization). Così assistant-request ci chiede la config al volo (voce, prompt orari/servizi/KB, tool prenotazione, maxDuration/gating). L'assistant statico "Puntify - Nemi" non serve al numero.
Voce: noi default Azure it-IT-ElsaNeural; se Stefano vuole 11labs → cambiare Vapi:DefaultVoiceProvider/Voice in config. Assistant di Stefano usa azure·anthropic·11labs.

## 2026-07-07 (17:30) — lingua account + allineamento config voce + config page TODO
Stefano ha mandato la config del suo assistant DEMO "Puntify - Nemi" (demo ristorante/parrucchiere + book appointment consulente): voce 11labs eleven_turbo_v2_5 voiceId v3V1d2rk6528UrLKRuy8 speed0.95 stab0.5 simil0.75 fallback vapi Emma; model anthropic claude-haiku-4-5-20251001 temp0.4; transcriber azure it-IT +fallback 11labs scribe_v1; endCallFunctionEnabled, forwardingPhoneNumber +393402606209, maxDuration473, server=n8n (loro). È il DEMO, diverso dal per-shop dinamico nostro.
RICHIESTA: lingua di risposta Nemi Voce = lingua ACCOUNT (default). IN CORSO (subagent a904c8874be666a2f): BuildVoiceContextAsync risolve owner_account_id→account.Language→prompt "rispondi di default in {lingua}"; + allinea config assistant-request ai valori Stefano (voce 11labs+voiceId, model anthropic claude-haiku temp0.4, transcriber azure locale-account, endCall+denoising) in VapiWebhookController+appsettings Vapi. Limite: voce resta IT anche se lingua account diversa finché non mappiamo voci per lingua.
TODO FUTURO (Stefano): pagina /merchant/{id}/assistantai/nemivoce per configurare lingue supportate da Nemi + parametri (voce/modello/informativa/saluto). Da progettare.

## FATTO (subagent a904c8874be666a2f) — lingua account + config voce
BuildVoiceContextAsync ora ritorna NemiVoiceContext(System, LanguageIso): risolve shop.OwnerAccountId→account.Language (fallback it), helper LanguageDisplayName, prompt blocco "LINGUA (vincolante): rispondi di default in {lang}". HandleAssistantRequest: voice 11labs(voiceId v3V1d2rk..., eleven_turbo_v2_5, speed0.95/stab0.5/simil0.75, fallback vapi Emma), model anthropic claude-haiku-4-5 temp0.4, transcriber azure MapLangToLocale(account)+fallback 11labs scribe_v1, endCallFunctionEnabled/endCallMessage "Arrivederci!"/backgroundDenoisingEnabled. appsettings Vapi Default* aggiornati (anthropic/claude-haiku/temp0.4/11labs/voiceId/eleven_turbo). Build 0 err, server :8001 riavviato, vapi 401 ok.
LIMITI: voce resta italiana anche per account non-it (no mappa voce-per-lingua → pagina config futura); endCallMessage fisso IT; serve creds anthropic+11labs su org Vapi.
